...
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
    ...
    if (request.action === 'sendWheelQuest') { // 命运转盘
        ...
        for (let i = 3; i < eqInfo.length ;i++) {
            ...
            var matchedKey = null;
            for (var key in eventQuestMap) {
                if (eqInfo[i].includes(key)) {
                    matchedKey = key;
                    break;
                }
            }
            if (!matchedKey || !eventQuestMap[matchedKey]) continue;
            var entry = eventQuestMap[matchedKey];
            var typeRow = eqot.insertRow();
            var typeERow = eqot.insertRow();
            var typeName = typeRow.insertCell();
            typeName.textContent = entry.label || matchedKey;
            var typeEName = typeERow.insertCell();
            typeEName.textContent = entry.labelE;
            for (let le = l1; le <= l2; le++) {
                var cell = typeRow.insertCell();
                cell.textContent = entry.data?.[le] ?? '';
                var cellE = typeERow.insertCell();
                cellE.textContent = entry.dataE?.[le] ?? '';
            }
        }
...
// event quest data moved to eventQuestData.js
